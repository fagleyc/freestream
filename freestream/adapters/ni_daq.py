...
from ni_usb_6351 import balcal                               # noqa: E402
...
class NiDaqAdapter(ConfigurableAdapter):
    ...
    def extra_meta(self) -> Dict[str, object]:
        ...
        * for this INTERNAL balance (bridge volts, NOT the external ATE),
          the COMPUTED calibration (``cal_matrix``/``cal_type``/
          ``cal_distances``/``balance_serial``/``balance_type``) so
