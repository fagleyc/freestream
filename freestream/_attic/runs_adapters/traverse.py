...
* ``settled()`` is "no axis moving or jogging" per ``drive.state()``
  (limit trips and stall aborts clear those flags, so a blocked move
  reports settled rather than hanging a sweep — check positions()).
        ...
        if uncal:
            return DeviceStatus(
                ...
                message=f"uncalibrated: {'/'.join(uncal)} — position "
                        f"moves disabled")
        limits = [n for n, s in state.items()
                  if s["enabled"] and s["limit"]]
        msg = (f"limit switch made on {'/'.join(limits)}"
               if limits else "")
        return DeviceStatus(state=OK, sim=self._sim, message=msg)
