...
def test_config_round_trip(tmp_path):
    ...
    assert back.right.unit == "kPa"
    assert back.left.role == "temperature" and back.left.unit == "C"


def test_default_port_order_temperature_left_pressure_right():
    """Regression guard (bench-confirmed TWICE: 73.61,11.43 on
    2026-07-23 and 74,11.3 on 07-24): the instrument transmits ``?``
    values TEMPERATURE first, PRESSURE second — 74 is room deg F and
    11.3 is ambient psi at altitude, never the other way around. The
    default port layout MUST match or the display swaps labels."""
    cfg = HeiseConfig()
    assert cfg.left.role == "temperature" and cfg.left.unit == "F"
    assert cfg.right.role == "pressure" and cfg.right.unit == "psi"
    # from_dict defaults (used when a saved bundle omits a port) match
    back = HeiseConfig.from_dict({"com_port": "COM1"})
    assert back.left.role == "temperature"
    assert back.right.role == "pressure"


def test_sim_reply_is_temperature_then_pressure():
    """The emulator's ``?`` reply order matches the real instrument:
    values[0] ~= room temperature (deg F), values[1] ~= ambient psi."""
    ...
    vals = p.read_values()
    assert 70.0 < vals[0] < 75.0      # temperature first (~room degF)
    assert 14.0 < vals[1] < 15.5      # pressure second (~ambient psi)
...
def test_connect_and_poll(dev):
    ...
    # order bench-confirmed twice: temperature LEFT, pressure RIGHT
...
def test_set_pressure_unit_live(dev):
    ...
    assert name == "kPa"
    assert dev.config.right.unit == "kPa"      # pressure port is RIGHT
...
def test_single_port_config():
    ...
    cfg = HeiseConfig(force_sim=True, poll_s=0.05)
    cfg.left.role = "off"                  # temperature off (LEFT port)
    d = HeiseGauge(cfg)
    try:
        d.connect()
...
def test_connect_with_rtd_port_survives_eunit(dev):
    """The RTD (LEFT port) rejects pressure unit codes (Err02).
    Units must be read-modify-write and non-fatal, so connect
    survives."""
    dev.connect()                       # raised HeiseError before fix
    assert dev.connected
    codes = dev.get_unit_codes()
    assert codes[0] == 15               # RTD code (left) untouched
    assert codes[1] == 0                # configured psi applied (right)
...
def test_wrong_pressure_role_on_rtd_port_falls_back():
    ...
    cfg = HeiseConfig(force_sim=True, poll_s=0.05)
    cfg.right.role = "temperature"      # right is physically the pressure port
    cfg.left.role = "pressure"          # wrong — the LEFT port is an RTD
    cfg.left.unit = "kPa"
    d = HeiseGauge(cfg)
    msgs = []
