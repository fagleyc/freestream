"""Help/About coverage for ac_delta: about metadata, Help menu, docs."""
...
from ac_delta import about

DOCS = Path(__file__).resolve().parents[1] / "ac_delta" / "docs" / "index.html"
...
def test_package_version_matches_about():
    import ac_delta
    assert ac_delta.__version__ == about.__version__
...
def test_help_menu_and_about_dialog():
    ...
    from ac_delta.app.main_window import CrescentMainWindow, _about_dialog
    from ac_delta.config import CrescentConfig

    app = QApplication.instance() or QApplication([sys.argv[0]])
    win = CrescentMainWindow(CrescentConfig(force_sim=True))
    try:
