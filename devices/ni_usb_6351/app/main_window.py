"""Main window + coordinator panel for the NI USB-6351 app.

Same structure as the StrainBook app: the driver owns the ring buffer,
the GUI pulls from it on a 10 Hz timer, status is marshalled via a Qt
signal.
"""
...
from ni_usb_6351 import about, theme
...
class NiDaqPanel(QWidget):
    """The complete NI USB-6351 GUI (also embeddable in host suites).

    ``device``/``embedded`` support hosting the EXACT same panel inside
    Freestream: pass the host's live :class:`NiUsb6351` so only ONE
    ...
    def __init__(self, cfg: Optional[NiDaqConfig] = None, parent=None,
                 *, device: Optional[NiUsb6351] = None,
                 embedded: bool = False):
        ...
        if device is not None:
            ...
            self.config = cfg or NiDaqConfig()
            self.device = NiUsb6351(self.config)
    ...
    def _build_ui(self):
        ...
        self.name_edit.setToolTip("NI-MAX device alias (e.g. Dev2)")
        cl.addWidget(self.name_edit)
        cl.addWidget(QLabel("Rate"))
        self.rate_spin = QDoubleSpinBox()
        self.rate_spin.setRange(1.0, 1_250_000.0)
        ...
        cl.addStretch(1)
        self.trig_lamp = QLabel("")
        self.trig_lamp.setProperty("mono", "true")
        self.trig_lamp.setStyleSheet(f"color: {theme.TEXT_DIM};")
        cl.addWidget(self.trig_lamp)
    ...
    def _update_trig_lamp(self):
        """Small trigger-state readout beside the main lamp."""
        trig = self.config.trigger
        if trig.mode == "immediate":
            text, color = "", theme.TEXT_DIM
        elif not self.device.connected:
            text, color = f"trig: {trig.source}", theme.TEXT_DIM
        elif self.device.waiting_for_trigger:
            text, color = f"ARMED — {trig.source}", theme.WARNING
        else:
            text, color = "Triggered", theme.SUCCESS
        if text != self.trig_lamp.text():
            self.trig_lamp.setText(text)
    ...
    def _refresh_ui(self):
        ...
        self._update_trig_lamp()
        if not connected:
            return
        waiting = self.device.waiting_for_trigger
        if waiting != self._last_waiting:      # armed → triggered lamp flip
...
class NiDaqMainWindow(QMainWindow):
    def __init__(self, cfg: Optional[NiDaqConfig] = None):
        super().__init__()
        self.setWindowTitle("NI USB-6351 — Analog I/O DAQ")
        self.resize(1200, 800)
        self.setStyleSheet(theme.get_stylesheet())
        self.panel = NiDaqPanel(cfg, self)
    ...
    def _load_cfg(self):
        path, _ = QFileDialog.getOpenFileName(self, "Load config", "",
                                              "JSON (*.json)")
        if path:
            self.panel.config = NiDaqConfig.load(path)
            self.panel.device.config = self.panel.config
            self.panel.forces_panel.config = self.panel.config
            self.panel.channels_panel._cfg = self.panel.config
            self.panel.channels_panel.reload()
            self.panel.output_panel._cfg = self.panel.config
            self.panel.output_panel.reload()
            self.panel.apply_settings()
